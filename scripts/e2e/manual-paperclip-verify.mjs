...
const stateRoot = persistentStateRoot ?? await mkdtemp(join(tmpdir(), 'paperclip-micronaut-plugin-manual-'));
...
async function ensureCompanySeeded() {
  ...
  const createdCompany = await fetchJson(companiesUrl, {
    ...
    body: JSON.stringify({
      name: 'Dummy Company',
      description: 'Seed company for manual paperclip-micronaut-plugin verification.'
    })
  });
...
async function ensurePluginInstalled(configPath) {
  try {
    ...
    const message = error instanceof Error ? error.message : String(error);
    if (message.includes('Plugin already installed: paperclip-micronaut-plugin')) {
