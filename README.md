...
When a company enables the experimental `enableExternalObjects` flag, any GitHub pull request URL posted in a merge-up issue's thread also gets a host-native external-object status chip (resolved by Paperclip's own GitHub provider), independent of and in addition to this plugin's own **PR chips** described above. The two are not the same mechanism and are not guaranteed to agree: the plugin's chip is scoped to the tracked merge-up row and reflects the plugin's own read of the linked pull request, while the host's chip is a generic per-comment resolution that applies to any PR URL in any issue thread. Operators should not expect the two to always be in lockstep if `enableExternalObjects` is on.

2026.831.1 also added an `idempotencyKey` field to Paperclip's issue-create API to deduplicate retried/concurrent creates. The plugin's merge-up **assignment wakeup** call already passes one (`micronaut-merge-up:<issueId>`), so repeated wakeup requests for the same tracked issue dedupe host-side. The merge-up **issue creation** call does not yet pass one: `@paperclipai/plugin-sdk@2026.831.1`'s `ctx.issues.create()` does not forward `idempotencyKey` to the host (only `requestWakeup`/`requestWakeups` do), so this plugin still relies on its existing plugin-state existence check to avoid duplicate merge-up issues, which is not fully race-proof under concurrent "Start merge up" clicks. See the comment above the `ctx.issues.create()` call in `src/worker.ts` for details; this will be revisited once the SDK exposes the field on issue creation.
